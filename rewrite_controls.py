...
# 1. Remove the old "Manual Batter Control"
old_manual_start = "{/* Manual Batter Control */}"
old_manual_end = "<div className=\"mt-4 pt-4 border-t w-full\">"
manual_start_idx = text.find(old_manual_start)
manual_end_idx = text.find(old_manual_end)
if manual_start_idx != -1 and manual_end_idx != -1:
    text = text[:manual_start_idx] + text[manual_end_idx:]

# 2. Rewrite the Umpire Controls grid
...
if start_idx != -1 and end_idx != -1:
    # Need to keep the spaces appropriately
    text = text[:start_idx] + new_grid + text[end_idx:]
